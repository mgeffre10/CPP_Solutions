#include "Dungeon.h"
#include "Enums.h"
#include "Random.h"
#include "Structs.h"

#include <cmath>
#include <iostream>
#include <set>

void Dungeon::generateRoomTypes(int numOfRooms)
{
	std::cout << "Generate room types\n";
	
	int gemShardRoomCount{ 4 };
	int enemyRoomCount{ static_cast<int>(std::floor(numOfRooms * .4)) };
	int potionRoomCount{ static_cast<int>(std::ceil(numOfRooms * .4) - gemShardRoomCount) }; // Want 40% of rooms to be "Item" rooms
	
	int currentIndex{ 0 };

	Room room{ RoomType::Start };
	m_rooms.insert({ room, std::map<char, int>{} });
	++currentIndex;

	std::cout << "Start room populated.\n";

	while (gemShardRoomCount > 0)
	{
		Room gemRoom{ RoomType::GemShard };
		m_rooms.insert({ gemRoom, std::map<char, int>{} });
		--gemShardRoomCount;
		++currentIndex;
	}

	std::cout << "Gem Shard rooms populated.\n";
	
	while (enemyRoomCount > 0)
	{
		Room enemyRoom{ RoomType::Enemy };
		m_rooms.insert({ enemyRoom, std::map<char, int>{} });
		--enemyRoomCount;
		++currentIndex;
	}

	std::cout << "Enemy rooms populated.\n";

	while (potionRoomCount > 0)
	{
		Room potionRoom{ RoomType::Potion };
		m_rooms.insert({ potionRoom, std::map<char, int>{} });
		--potionRoomCount;
		++currentIndex;
	}

	std::cout << "Potion rooms populated.\n";

	while (currentIndex < numOfRooms - 1)
	{
		Room emptyRoom{ };
		m_rooms.insert({ emptyRoom, std::map<char, int>{} });
		++currentIndex;
	}

	Room exitRoom{ RoomType::Exit };
	m_rooms.insert({ exitRoom, std::map<char, int>{} });

	std::cout << "Exit room populated.\n";
}

void Dungeon::generateAdjacentRooms()
{
	std::cout << "Generate Adjacent Rooms\n";
	
	for (auto &entry : m_rooms)
	{
		int connectionAttempts{ 1 };
		bool shouldAddAnotherRoom{ true };
		Room currentRoom{ entry.first };
		std::map<char, int> adjRooms{ entry.second };

		if ((currentRoom.type != RoomType::Exit && adjRooms.size() == 4) || (currentRoom.type == RoomType::Exit && adjRooms.size() == 3)) continue;
		while (connectionAttempts < 4 && shouldAddAnotherRoom) // Don't want this to go on forever
		{
			int minValue{ 1 };
			int maxValue{ static_cast<int>(m_rooms.size()) };
			int roomIdToAdd{ Random::getRandomNumberInRange(minValue, maxValue) };
			auto room{ getRoomById(roomIdToAdd) };
			
			auto roomAlreadyAdjacent{ std::find_if(entry.second.begin(), entry.second.end(), [roomIdToAdd](auto pair) { return pair.second == roomIdToAdd; }) };

			while (roomIdToAdd == currentRoom.id
				|| roomAlreadyAdjacent != entry.second.end()
				|| (room.first.type == RoomType::Exit && static_cast<int>(room.second.size()) == 3)
				|| (room.first.type != RoomType::Exit && static_cast<int>(room.second.size()) == 4)
				|| room.first.type == currentRoom.type
				|| (currentRoom.id == 1 && roomIdToAdd == static_cast<int>(m_rooms.size())))
			{
				roomIdToAdd = Random::getRandomNumberInRange(minValue, maxValue);
				room = getRoomById(roomIdToAdd);
				roomAlreadyAdjacent = std::find_if(entry.second.begin(), entry.second.end(), [roomIdToAdd](auto pair) { return pair.second == roomIdToAdd; });
			}

			std::vector<char> directions{ getOpenDirections(entry.second, room.second) };

			std::cout << "Current open directions for room " << currentRoom.id << ": ";
			for (const char direction : directions)
			{
				std::cout << direction << ' ';
			}

			std::cout << '\n';

			std::pair<char, int> roomToAdd{};

			if (directions.size() == 0)
			{
				std::cout << "No directions available, trying again. Current attempt: " << connectionAttempts << '\n';
				++connectionAttempts;
				continue;
			}
			else if (directions.size() == 1)
			{
				roomToAdd.first = directions[0];
				roomToAdd.second = roomIdToAdd;
			}
			else
			{
				int randomDirection{ Random::getRandomNumberInRange(0, static_cast<int>(directions.size()) - 1) };

				roomToAdd.first = directions[randomDirection];
				roomToAdd.second = roomIdToAdd;
			}

			std::cout << "Adding room " << roomToAdd.second << " in direction " << roomToAdd.first << " to room " << currentRoom.id << '\n';

			entry.second.insert(roomToAdd);

			std::map<char, int> addedRoomAdjRooms{ getAdjacentRooms(roomIdToAdd) };

			if (roomToAdd.first == 'W' || roomToAdd.first == 'E')
			{
				auto northIndex{ entry.second.find('N') };
				auto southIndex{ entry.second.find('S') };

				if (northIndex != entry.second.end())
				{
					std::map<char, int> adjacentRooms{ getAdjacentRooms(northIndex->second) };

					auto newNeighbor{ adjacentRooms.find(roomToAdd.first) };
					if (newNeighbor != adjacentRooms.end())
					{
						std::cout << "Found new neighbor in the 'N' direction.\n";
						addedRoomAdjRooms.insert({ 'N', newNeighbor->second });
					}
				}

				if (southIndex != entry.second.end())
				{
					std::map<char, int> adjacentRooms{ getAdjacentRooms(southIndex->second) };

					auto newNeighbor{ adjacentRooms.find(roomToAdd.first) };
					if (newNeighbor != adjacentRooms.end())
					{
						std::cout << "Found new neighbor in the 'S' direction.\n";
						addedRoomAdjRooms.insert({ 'S', newNeighbor->second });
					}
				}
			}
			else
			{
				auto westIndex{ entry.second.find('W') };
				auto eastIndex{ entry.second.find('E') };

				if (westIndex != entry.second.end())
				{
					std::map<char, int> adjacentRooms{ getAdjacentRooms(westIndex->second) };

					auto newNeighbor{ adjacentRooms.find(roomToAdd.first) };
					if (newNeighbor != adjacentRooms.end())
					{
						std::cout << "Found new neighbor in the 'W' direction.\n";
						addedRoomAdjRooms.insert({ 'W', newNeighbor->second });
					}
				}

				if (eastIndex != entry.second.end())
				{
					std::map<char, int> adjacentRooms{ getAdjacentRooms(eastIndex->second) };

					auto newNeighbor{ adjacentRooms.find(roomToAdd.first) };
					if (newNeighbor != adjacentRooms.end())
					{
						std::cout << "Found new neighbor in the 'E' direction.\n";
						addedRoomAdjRooms.insert({ 'E', newNeighbor->second });
					}
				}
			}

			switch (roomToAdd.first)
			{
				case 'N':
					std::cout << "Adding room " << currentRoom.id << " in direction S to room " << roomToAdd.second << '\n';
					addedRoomAdjRooms.insert({ 'S', currentRoom.id });
					break;
				case 'S':
					std::cout << "Adding room " << currentRoom.id << " in direction N to room " << roomToAdd.second << '\n';
					addedRoomAdjRooms.insert({ 'N', currentRoom.id });
					break;
				case 'W':
					std::cout << "Adding room " << currentRoom.id << " in direction E to room " << roomToAdd.second << '\n';
					addedRoomAdjRooms.insert({ 'E', currentRoom.id });
					break;
				case 'E':
					std::cout << "Adding room " << currentRoom.id << " in direction W to room " << roomToAdd.second << '\n';
					addedRoomAdjRooms.insert({ 'W', currentRoom.id });
					break;
				default:
					std::cerr << "Error, unknown direction passed.\n";
					return;
			}
			
			std::cout << "Number of adjacent rooms for room " << currentRoom.id << ": " << entry.second.size() << '\n';
			if (static_cast<int>(entry.second.size()) == 4) shouldAddAnotherRoom = false;
			else if (static_cast<int>(entry.second.size()) >= 2)
			{
				if (currentRoom.type != RoomType::Exit)
				{
					if (entry.second.size() < 4)
					{
						shouldAddAnotherRoom = Random::getRandomNumberInRange(0, 1);
					}
				}
				else
				{
					if (entry.second.size() < 3)
					{
						shouldAddAnotherRoom = Random::getRandomNumberInRange(0, 1);
					}
				}
			}

		}
	}
}

std::map<char, int> Dungeon::getAdjacentRooms(int roomId)
{
	return getRoomById(roomId).second;
}

std::vector<char> Dungeon::getOpenDirections(std::map<char, int> currentRoom, std::map<char, int> adjRoom)
{
	std::vector<char> openDirections{ 'N', 'S', 'E', 'W'};

	int currentRoomSize{ static_cast<int>(currentRoom.size()) };
	int adjRoomSize{ static_cast<int>(adjRoom.size()) };
	if (currentRoomSize != 0 || adjRoomSize != 0)
	{
		if (currentRoomSize > 0)
		{
			for (const auto entry : currentRoom)
			{
				auto iter{ std::find_if(openDirections.begin(), openDirections.end(), [entry](char value) { return value == entry.first; }) };
				
				if (iter != openDirections.end()) openDirections.erase(iter);
			}
		}

		if (adjRoomSize > 0)
		{
			for (const auto entry : adjRoom)
			{
				std::vector<char>::iterator iter{};

				switch (entry.first)
				{
					case 'N':
						iter = std::find_if(openDirections.begin(), openDirections.end(), [entry](char value) { return value == 'S'; });
						if (iter != openDirections.end()) openDirections.erase(iter);
						break;
					case 'S':
						iter = std::find_if(openDirections.begin(), openDirections.end(), [entry](char value) { return value == 'N'; });
						if (iter != openDirections.end()) openDirections.erase(iter);
						break;
					case 'W':
						iter = std::find_if(openDirections.begin(), openDirections.end(), [entry](char value) { return value == 'E'; });
						if (iter != openDirections.end()) openDirections.erase(iter);
						break;
					case 'E':
						iter = std::find_if(openDirections.begin(), openDirections.end(), [entry](char value) { return value == 'W'; });
						if (iter != openDirections.end()) openDirections.erase(iter);
						break;
					default:
						std::cerr << "Unexpected value, exiting.\n";
						return {};
				}
			}
		}
	}

	
	
	return openDirections;
}

std::pair<Room, std::map<char, int>> Dungeon::getRoomById(int roomId)
{	
	auto room{ std::find_if(m_rooms.begin(), m_rooms.end(),[roomId](auto pair) {return pair.first.id == roomId; }) };
	
	if (room != m_rooms.end()) return *room;
	return {};
}

void Dungeon::printMap()
{
	std::cout << "Printing map\n";

	for (const auto &entry : m_rooms)
	{
		std::cout << "Room ID: " << entry.first.id << '\n';
		std::cout << "Room Type: " << Enums::printRoomType(entry.first.type) << '\n';
		std::cout << "Adjacent Rooms: ";

		for (const auto &room : entry.second)
		{
			std::cout << room.second << room.first << ' ';
		}

		std::cout << '\n';
	}
}